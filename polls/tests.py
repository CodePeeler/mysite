...
class PollMethodTests(TestCase):
    ...
    def test_was_published_recently_with_future_poll(self):
        """
        was_published_recently() should return False for polls whose pub date is in the future
        """
...
def create_poll(question, days):
	"""
	Creates a poll with the fiven 'question' published the fiven number of 'days' offset to now (negative for polls published in the past, positive for polls that have yet to be published).
	"""
...
class PollViewTests(TestCase):
    def test_index_view_with_no_polls(self):
		...
		#Recall that 'latest_poll_list' is the context object name as set in views.py
    ...
    def test_index_view_with_future_poll_and_past_poll(self):
	    """
	    Even if both past and future polls exist, only past polls should ve displayed.
	    """
	    create_poll(question="Past poll.", days=-30)
	    create_poll(question="Future poll.", days=30)
	    response = self.client.get(reverse('polls:index'))
